...
async def test_concurrent_put_same_key():
    ...
    try:
        key = b"/mx/race/worker/0"
        values = [f'{{"from":"node{i}"}}'.encode() for i in range(3)]

        # All three nodes PUT the same key simultaneously
        tasks = [nodes[i].put(key, values[i]) for i in range(3)]
        results = await asyncio.gather(*tasks)

        # All PUTs should succeed (returned count >= 0)
        for r in results:
            assert isinstance(r, int)

        # Reading from any node should return a valid value (one of the three)
        for node in nodes:
...
async def test_concurrent_put_different_keys():
    ...
    try:
        keys_values = [
            (f"/mx/model-{i}/worker/0".encode(), f'{{"rank":0,"model":{i}}}'.encode())
            for i in range(3)
        ]

        # Each node PUTs a different key simultaneously
        tasks = [nodes[i].put(k, v) for i, (k, v) in enumerate(keys_values)]
        await asyncio.gather(*tasks)

        # All keys should be retrievable from any node
