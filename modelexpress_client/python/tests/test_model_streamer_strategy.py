...
from modelexpress.load_strategy.base import IteratorCleanupError
...
class TestModelStreamerLoad:
    ...
    @patch("modelexpress.load_strategy.model_streamer_strategy.register_tensors")
    def test_iterator_cleanup_failure_stops_recovery(self, mock_register):
        model = MagicMock()
        adapter = _FakeAdapter()
        adapter.apply_weight_iter = MagicMock(side_effect=RuntimeError("partial load"))
        ctx = _make_load_context(adapter=adapter)
        weights_iter = MagicMock()
        weights_iter.close.side_effect = RuntimeError("streamer close failed")
        strategy = self._make_strategy()
        ...
        with patch.dict("os.environ", {"MX_MODEL_URI": "s3://bucket/model"}):
            with patch(
                ...
                with pytest.raises(IteratorCleanupError, match="streamer close failed"):
                    strategy.load(model, ctx)
