...
def test_load_skips_self_source_candidate_from_detected_local_host(monkeypatch):
    strat = RdmaStrategy()
    cands = _sources(2)
    strat._find_source_instances = MagicMock(return_value=cands)
    self_worker = p2p_pb2.WorkerMetadata(
    ...
    class FakeSocket:
        ...
        def getsockname(self):
            return ("10.0.0.9", 43210)

    monkeypatch.delenv("MX_WORKER_HOST", raising=False)
    monkeypatch.setattr(
        "modelexpress.load_strategy.rdma_strategy.socket.socket",
        lambda *args, **kwargs: FakeSocket(),
    )
    monkeypatch.setattr(
        "modelexpress.load_strategy.rdma_strategy.socket.getfqdn",
        lambda: "local.example",
    )
    monkeypatch.setattr(
        "modelexpress.load_strategy.rdma_strategy.socket.gethostname",
        lambda: "local",
    )
    monkeypatch.setattr(
        "modelexpress.load_strategy.rdma_strategy.socket.getaddrinfo",
        lambda host, port: [(None, None, None, None, (host, 0))],
    )
    monkeypatch.setattr(
        "modelexpress.load_strategy.rdma_strategy.worker_tensor_count",
        lambda w: 1,
