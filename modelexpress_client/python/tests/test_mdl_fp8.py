...
def test_fp8_tp2_expert_gate_up_weight_and_scale_shards(monkeypatch):
    ...
    gate = torch.tensor([[1, 2], [3, 4], [5, 6], [7, 8]], dtype=F8)
    up = torch.tensor([[11, 12], [13, 14], [15, 16], [17, 18]], dtype=F8)
    ...
    for rank in (0, 1):
        ...
        assert torch.equal(mlp.experts.w13_weight[0, :2].float(), gate[rows].float())
        assert torch.equal(mlp.experts.w13_weight[0, 2:].float(), up[rows].float())
        assert torch.equal(mlp.experts.w13_weight_scale[0, 0], gate_scale[rank])
        assert torch.equal(mlp.experts.w13_weight_scale[0, 1], up_scale[rank])
        assert torch.equal(mlp.experts.w2_weight[0].float(), down[:, cols].float())
        assert torch.equal(
