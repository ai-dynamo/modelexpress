...
async def test_periodic_rebootstrap_discovers_new_peers():
    """The periodic bootstrap loop should discover peers that joined
    after initial bootstrap, provided the responder has dial-verified
    those peers.

    v0.3.3 hardened the responder-side hint filter: peers known only via
    inbound connection (the responder accepted the connection but never
    outbound-dialled the peer's listen addrs) are no longer propagated
    in FIND_NODE / GET_VALUE response peer lists. This closes the
    long-running-responder ghost-peer leak (where dead joiners stayed
    ``connected=True`` forever in seed routing tables and got served
    back to fresh joiners as dead-but-advertised hints), at the cost of
    transitive discovery via inbound-only peers no longer working.
    Consumers that relied on the seed handing out a fresh joiner's
    address must now dial that joiner directly. v0.4.0 will restore
    transitive discovery via a tri-state verified/learned/failed peer
    model that distinguishes "we have heard of this peer" from "we have
    verified this peer" without permanently shadowing learnt-only peers.

    To exercise the periodic-rebootstrap discovery path under the new
    contract, this test promotes node_c to ``dial_verified`` in node_a's
    routing table after node_c bootstraps inbound. In production this
    promotion happens when node_a dials node_c (e.g., as part of node_a's
    own outbound bootstrap or maintenance loop); here we set the flag
    explicitly so the test stays focused on the periodic-rebootstrap
    discovery mechanism rather than on an additional dial path.
    """
    node_a = DhtNode(record_ttl=300)
    await node_a.start("127.0.0.1", 0)
    ...
    try:
        ...
        # Promote node_c to dial-verified in node_a's RT. Without this,
        # node_a treats node_c as inbound-only under the v0.3.3 filter
        # and excludes it from closer-peer hints, so node_b's re-bootstrap
        # walk would not learn about node_c via node_a.
        # Bounded poll so the test does not race on node_a's inbound-
        # connection bookkeeping (Identify exchange + add_or_update can
        # land slightly after the asyncio.sleep above).
        for _ in range(20):
            if node_a.routing_table.mark_dial_verified(node_c.peer_id):
                break
            await asyncio.sleep(0.05)
        else:
            pytest.fail(
                "node_c should be present in node_a's routing table within 1s"
            )
