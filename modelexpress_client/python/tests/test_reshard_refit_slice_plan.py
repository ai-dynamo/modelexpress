...
from modelexpress.reshard_refit.slice_plan import Shard, op_chain_to_box, plan_pull
from modelexpress.reshard_refit.types import RecordedCopy, UnsupportedReshard
...
def test_column_block_single_contiguous_run():
    ...
    segs = plan_pull(copy, global_shape=(8, 4), src_dtype=F32, elsize=EL, shards=[shard])
    assert len(segs) == 1
    assert segs[0].src_addr == 1000 and segs[0].dst_byte == 0 and segs[0].nbytes == 16 * EL
...
def test_column_slice_is_strided_multi_run():
    ...
    segs = plan_pull(copy, global_shape=(4, 8), src_dtype=F32, elsize=EL, shards=[shard])
...
def test_fused_shard_dest_offset():
    ...
    segs = plan_pull(copy, global_shape=(2, 4), src_dtype=F32, elsize=EL, shards=[shard])
    assert len(segs) == 1
...
def test_multi_shard_fan_in():
    ...
    segs = plan_pull(copy, global_shape=(4, 8), src_dtype=F32, elsize=EL, shards=[left, right])
...
def _reconstruct(src_2d, copy, shards, segs):
    ...
    return dst_buf[copy.dest_offset : copy.dest_offset + dest_numel].reshape(copy.dest_shape)
...
def test_transpose_reconstructs_bit_for_bit():
    ...
    segs = plan_pull(copy, global_shape=(3, 4), src_dtype=F32, elsize=EL, shards=[shard])
...
def test_permute_3d_reconstructs_bit_for_bit():
    ...
    shard = Shard(shard_offset=(0, 0, 0), shape=(2, 3, 4), session="s0", addr=0, elsize=EL)
    segs = plan_pull(copy, global_shape=(2, 3, 4), src_dtype=F32, elsize=EL, shards=[shard])
    assert torch.equal(_reconstruct(src, copy, [shard], segs), ground_truth)
...
def test_transpose_of_narrowed_block_reconstructs():
    ...
    copy = _copy((("narrow", (0, 1, 2), ()), ("transpose", (0, 1), ())), "wNT", 0, (5, 2), (2, 1))
    shard = Shard(shard_offset=(0, 0), shape=(4, 5), session="s0", addr=0, elsize=EL)
    segs = plan_pull(copy, global_shape=(4, 5), src_dtype=F32, elsize=EL, shards=[shard])
    assert torch.equal(_reconstruct(src, copy, [shard], segs), ground_truth)
...
def test_transpose_then_contiguous_materializes_falls_back():
    ...
    copy = _copy((("transpose", (0, 1), ()), ("contiguous", (), ())), "w", 0, (4, 4), (4, 1))
...
def test_dtype_mismatch_raises():
    ...
    shard = Shard(shard_offset=(0, 0), shape=(4, 4), session="s0", addr=0, elsize=EL)
    with pytest.raises(UnsupportedReshard):
        plan_pull(copy, global_shape=(4, 4), src_dtype="bf16", elsize=EL, shards=[shard])
