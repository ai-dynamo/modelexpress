...
# ---------------------------------------------------------------------------
# mDNS auto-disable: kademlite turns mDNS off when bootstrap is wired,
# but only via args passed to DhtNode.start(). MxDhtClient configures
# bootstrap via separate calls afterward, so it has to set enable_mdns
# itself or mDNS leaks discovery across the boundaries the bootstrap
# layer is enforcing.
# ---------------------------------------------------------------------------


def test_mdns_disabled_when_bootstrap_dns_configured(monkeypatch):
    """Bootstrap DNS configured -> mDNS off in the underlying node."""
    monkeypatch.setenv("MX_DHT_LISTEN", "127.0.0.1:0")
    monkeypatch.setenv("MX_DHT_BOOTSTRAP_DNS", "no-such-host.invalid")
    monkeypatch.delenv("MX_DHT_BOOTSTRAP_PEERS", raising=False)
    monkeypatch.delenv("MX_DHT_BOOTSTRAP_SLURM", raising=False)
    ...
    try:
        ...
        assert client._node._mdns is None, (
            "mDNS should be disabled when bootstrap DNS is configured"
        )
    finally:
        client.close()


def test_mdns_disabled_when_bootstrap_peers_configured(monkeypatch):
    """Explicit peer multiaddrs -> mDNS off."""
    monkeypatch.setenv("MX_DHT_LISTEN", "127.0.0.1:0")
    monkeypatch.setenv(
        "MX_DHT_BOOTSTRAP_PEERS",
        "/ip4/127.0.0.1/tcp/65535/p2p/12D3KooWGRUacD9P3o6gE7nU5kuKfTeZBtfYS9bQ87rYTixGnyqx",
    )
    monkeypatch.delenv("MX_DHT_BOOTSTRAP_DNS", raising=False)
    monkeypatch.delenv("MX_DHT_BOOTSTRAP_SLURM", raising=False)
    monkeypatch.delenv("SLURM_JOB_NODELIST", raising=False)
    ...
    try:
        client._ensure_started()
        assert client._node._mdns is None, (
            "mDNS should be disabled when bootstrap peers are configured"
        )
    finally:
        client.close()


def test_mdns_enabled_when_no_bootstrap(monkeypatch):
    """No bootstrap of any kind -> mDNS auto-enables (single-host fallback)."""
    monkeypatch.setenv("MX_DHT_LISTEN", "127.0.0.1:0")
    monkeypatch.delenv("MX_DHT_BOOTSTRAP_PEERS", raising=False)
    monkeypatch.delenv("MX_DHT_BOOTSTRAP_DNS", raising=False)
    monkeypatch.delenv("MX_DHT_BOOTSTRAP_SLURM", raising=False)
    ...
    try:
        client._ensure_started()
        assert client._node._mdns is not None, (
            "mDNS should auto-enable when no bootstrap is configured"
        )
    finally:
        client.close()
