...
def test_s3_full_hf_checkpoint_upload_failure_converges_before_manifest_gather(
    monkeypatch, tmp_path, refit_server
):
    service, server_url = refit_server
    service.target.payload_format = refit_pb2.WEIGHT_PAYLOAD_FORMAT_FULL_HF_CHECKPOINT
...
def test_s3_full_hf_checkpoint_remote_index_failure_prevents_state_transition(
    monkeypatch, tmp_path, refit_server
):
    service, server_url = refit_server
    service.target.payload_format = refit_pb2.WEIGHT_PAYLOAD_FORMAT_FULL_HF_CHECKPOINT
    ...
    def all_gather(output, value, group=None):
        ...
        if isinstance(value, tuple):
            output[:] = [(0, 1), (1, 1)]
        elif all_gather_calls == 2:
            output[:] = [None, None]
        else:
            output[:] = ["RuntimeError: injected index failure", None]
