...
"""Real-model PrimeRL weight-transfer tests (requires 2 CUDA GPUs + Qwen2.5-0.5B).

Loads Qwen/Qwen2.5-0.5B (290 params, 988 MB bfloat16) on two GPUs and validates
the full pipeline: bake/resolve/route → RDMA descriptors → cudaMemcpy.
"""
...
def _build_regions(table: TrainerTable, inf_params: dict[str, torch.Tensor]) -> list[ResolvedRegion]:
    ...
    for copy in recorder.copies:
        ...
        regions.append(ResolvedRegion(
            tensor_name=copy.src_name, src_elem_runs=src_runs,
            dst_addr=dst_param.data_ptr(), dst_elem_runs=dst_runs,
            element_size=copy.dst_dtype.itemsize, dst_device_id=1,
        ))
    return regions
...
class TestRealModelFSDP:
    ...
    def test_fsdp_table_tensor_count(self, trainer_params):
        assert len(_fsdp_table(trainer_params).tensors) == N_PARAMS
    ...
    def test_fsdp_throughput_above_floor(self, trainer_params, inf_params, libcudart):
        ...
        assert total_bytes / elapsed / 1e9 > 5.0
...
class TestRealModelTP2:
    ...
    def test_tp2_col_sharded_params_correct(self, trainer_params, inf_params, libcudart):
        ...
        rp_bad = [n for n in rp_names if n in _mismatches(trainer_params, inf_params)]
