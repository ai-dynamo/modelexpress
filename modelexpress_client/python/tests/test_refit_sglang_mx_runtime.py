...
def test_sglang_mx_runtime_source_ownerships_are_filterable_per_pod():
    owners = build_sglang_mx_runtime_source_ownerships(
    ...
    assert owners[0].layout_tags["runtime_refit_target"] == "sglang.Engine"

    rank1 = _filter_source_ownerships(owners, source_id="trainer-rank1")
    assert [owner.source_id for owner in rank1] == ["trainer-rank1"]


def test_sglang_mx_runtime_source_publications_keep_metadata():
    owners = build_sglang_mx_runtime_source_ownerships(
        tensor_name="lm_head.weight",
        shape=(6, 4),
        dtype_name="float32",
    ...
    publications = materialize_sglang_mx_runtime_source_publications(
        owners,
        dtype=torch.float32,
...
def test_sglang_mx_runtime_distributed_publisher_defaults_to_current_rank_filter():
...
def test_sglang_mx_runtime_scenario_uses_mx_endpoint_ownerships_for_plan():
    ...
    mx_owners = build_sglang_mx_runtime_source_ownerships(
        tensor_name="lm_head.weight",
        shape=(8, 4),
        dtype_name="float32",
        model_name="mx-sglang-mx-unit",
    ...
    assert context["plan_source"] == "live-sglang-request+mx-endpoint-ownerships"
