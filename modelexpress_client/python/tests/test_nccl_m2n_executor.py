...
"""No-GPU failure-atomicity tests for the nccl_m2n executor."""
...
class FakeM2N:
    ...
    def memcpy_dtod(self, dst_ptr: int, src_ptr: int, nbytes: int) -> None:
        self.events.append(("copy", dst_ptr, src_ptr, nbytes))
    ...
    def reshard(self, comm, window, src, dst, stream=0) -> None:
        ...
        self.events.append(("reshard", call))
...
def _destination_executor(fake: FakeM2N) -> NcclM2nExecutor:
...
def test_destination_commits_only_after_complete_version_is_staged():
    ...
    executor = _destination_executor(fake)
    ...
    first_live_copy = min(
        ...
        if event[0] == "copy" and event[1] in fake.live_ptrs
    )
