...
| `MX_RESHARD_SPREAD_SOURCES` | `0` | Rotate which byte-identical DP/EDP replica serves each shard, by receiver global rank, instead of every receiver reading from the first publisher discovered. Changes only the owning agent and address of each shard, never the plan's shape, segment count or byte count. Off by default: it is only a win once the per-session distribution says one publisher is the bottleneck, and until the digest consumer lands a non-zero offset lets different receivers resolve a publisher name collision differently |
