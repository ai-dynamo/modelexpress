...
| `MX_LOAD_STRATEGY_CHAIN` | `INFERENCE` | Select the engine-neutral initial-load policy. Set to `RL` for desired-version P2P, desired-version S3 full-plus-delta replay, `MX_MODEL_URI`, then engine-default fallback |
| `MX_REFIT_DESIRED_VERSION_UID` | (unset) | Exact version requested when `MX_LOAD_STRATEGY_CHAIN=RL`; desired-version strategies skip when it is unset |
