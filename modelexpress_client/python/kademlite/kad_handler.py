...
class KadHandler:
    ...
    def _closest_peers_encoded(self, target: bytes, count: int | None = None) -> list[bytes]:
        ...
        Filters to ``dial_verified_only=True`` so FIND_NODE / GET_VALUE
        responses only propagate routing-table entries we have personally
        outbound-dialled at their listen addrs. The v0.3.2
        ``connected_only`` filter excluded entries the local node had
        explicitly marked unreachable, but ``connected=True`` is too
        permissive a signal: long-running responder nodes accumulate
        ``connected=True`` entries from inbound connections (where the
        peer's listen addrs were learnt via Identify but never directly
        dialled by the local node) and from FIND_NODE / GET_VALUE
        response hints (added to the routing table by the iterative-walk
        integration step at default-True ``connected`` without a
        verifying dial). Both populations pass the ``connected_only``
        filter even though their listen addrs are unverified, so fresh
        joiners bootstrapping against a long-running responder still
        inherit dead-peer hints and pay one ``DIAL_TIMEOUT`` per ghost
        in alpha-sized batches before subsequent operations can run.
        The ``dial_verified`` bit narrows the responder filter to
        entries an authoritative outbound-dial-success signal has
        touched, so dead-but-unverified entries can no longer leak.
        ...
        if count is None:
            count = self._k
        entries = self.routing_table.closest_peers(
            target, count, dial_verified_only=True
        )
        result = []
