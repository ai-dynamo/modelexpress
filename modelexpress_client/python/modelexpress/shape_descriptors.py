...
class NonExpertShardSpec:
    ...
    tensor so receivers can pull + reassemble across ranks instead of the
    publisher pre-gathering the full tensor.
...
def describe_tensor(
    *,
    ...
    # of a larger logical tensor (plain post-conversion shards that are no
    # longer DTensors, e.g. prime-rl's ShardedSlot buffers).
