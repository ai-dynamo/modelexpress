...
class MxV2TrainingPublisher:
    ...
    def _publish_reshard_view(self) -> str:
        ...
        manager = self._publisher._nixl
        client = self._publisher._client
        if manager is None or client is None:
            raise RuntimeError("underlying NIXL publisher is not initialized")
        endpoint = os.environ.get("MX_RESHARD_METADATA_ENDPOINT")
        if not endpoint:
            listen_port = getattr(manager, "_listen_port", None)
            if listen_port is None:
                raise RuntimeError(
                    "MX_MEGATRON_RESHARD496=1 requires a NIXL listen port or "
                    "MX_RESHARD_METADATA_ENDPOINT"
                )
            host = os.environ.get("POD_IP") or socket.gethostbyname(
                socket.gethostname()
            )
            endpoint = f"{host}:{int(listen_port)}"
        raw_name_map = self._megatron_sidecar.get("megatron_hf_name_map", [])
        name_map = {
            str(entry[0]): tuple(str(name) for name in entry[1])
            for entry in raw_name_map
        ...
        for descriptor in self._registry:
            lookup_name = descriptor.name.removeprefix("module.")
            hf_names = name_map.get(lookup_name) or name_map.get(descriptor.name)
            if not hf_names:
                raise RuntimeError(
                    f"no Megatron-to-HF alias mapping for {descriptor.name!r}"
                )
            items.append(
                MegatronAliasInput(
                    name=descriptor.name,
                    tensor=self._registered_tensors[descriptor.name],
                    role=str(descriptor.megatron_role),
                    hf_names=hf_names,
                    global_shape=tuple(
                        int(dim) for dim in descriptor.global_shape
                    ),
                    placement_kind=str(descriptor.placement_kind),
                    shard_axis=(
                        int(descriptor.shard_axis)
                        if descriptor.placement_kind == PLACEMENT_SHARD
                        else None
                    ),
                    local_shard_range=(
                        tuple(
                            int(value)
                            for value in descriptor.local_shard_range
                        )
                        if descriptor.placement_kind == PLACEMENT_SHARD
                        and descriptor.local_shard_range is not None
                        else None
                    ),
                    extras=dict(descriptor.megatron_extras or {}),
                )
            )
        aliases = build_hf_aliases(items, agent_name=str(manager.agent_name))
        source_id = publish_registered_shard_table(
            manager=manager,
            client=client,
            model_name=self._publisher._model_name,
            worker_rank=self._worker_rank,
