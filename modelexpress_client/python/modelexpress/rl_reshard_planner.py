...
The algorithm: for every :class:`SliceRequest`, intersect its global
range against every :class:`SliceOwnership` whose tensor name + dtype +
compile_target match. Emit one :class:`SegmentPlan` per non-empty
intersection. Surface gaps as ``CoveragePlan.missing_ranges`` so
callers can fall back to a full-copy install path for affected tensors.
"""
...
def _source_matches_request(
    ...
    # Expert-parallel filtering — only applies when the request explicitly
    # sets required_experts. With required_experts=None (default), expert
    # metadata is ignored entirely and the planner behaves as it did before
    # this filter was added. This preserves back-compat for callers that
    # don't use MoE EP.
