...
class WorkerServiceStub(object):
    """Worker-to-worker service running on each source worker.
    Targets connect directly to fetch tensor manifests (MB-scale data that
    should not transit the centralized discovery layer).
    """
...
class WorkerServiceServicer(object):
    ...
    def GetTensorManifest(self, request, context):
        """Fetch the tensor manifest for this worker's GPU.
        """
...
class P2pServiceStub(object):
    ...
    NIXL agent blobs are exchanged directly between peers via NIXL's native listen thread
    (send_local_metadata / fetch_remote_metadata), never stored centrally.
    Tensor manifests are served directly by source workers via WorkerService.
    """
...
class P2pServiceServicer(object):
    ...
    def GetMetadata(self, request, context):
        """Fetch lightweight directory metadata for one specific worker.
