...
class MxRefitReceiver:
    ...
    def receive_weights(
        ...
        # `receive_from_source` returns (total_bytes, total_tensors_matched,
        # duration). The second value is a tensor count, NOT a byte count.
        # `bytes_skipped` is derived from the gap between what the source
        # advertised and what we actually transferred.
        ...
        stats = TransferStats(
            ...
            bytes_skipped=max(advertised_bytes - transferred, 0),
            tensors_received=matched_tensors,
    ...
    def receive_weights_from_metadata(
        ...
        # Mirror the filter applied in `receive_weights` / `receive_weights_scratch`:
        # callers may include the V2 sidecar TensorDescriptors
        # (name="__mx_v2_meta__", addr=0, size=0) when forwarding raw catalog
        # metadata; passing those into prep_xfer_dlist trips UCX with a
        # (0,0,0) descriptor.
        real_source_tensors = [
            td for td in source_tensors
            if not td.name.startswith("__mx_") and td.size > 0
        ]

        # `receive_from_source` returns (total_bytes, total_tensors_matched,
        # duration); see `receive_weights` for the tuple-semantics rationale.
        transferred, matched_tensors, elapsed = self._nixl.receive_from_source(
            source_metadata=nixl_metadata,
            source_tensors=real_source_tensors,
            timeout_seconds=timeout_seconds,
        )
        advertised_bytes = sum(td.size for td in real_source_tensors)
        ...
        logger.info(
            ...
            f"{len(real_source_tensors)} tensors, {elapsed:.2f}s, "
            f"{stats.bandwidth_gbps:.1f} Gbps"
        )

        self._current_step = training_step

        for td in real_source_tensors:
            if td.name in self._nixl._tensors:
                yield td.name, self._nixl._tensors[td.name]
