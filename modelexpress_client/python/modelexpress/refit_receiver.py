...
class MxRefitReceiver:
    ...
    def __init__(
        ...
        self._scratch_arena: torch.Tensor | None = None
        self._scratch_arena_bytes: int = 0
        ...
        if self._scratch_sig is None:
            with refit_span(
                ...
                for td in all_source_tensors:
                    ...
                    required_bytes += int(td.size)
                configured_bytes = int(
                    float(os.environ.get("MX_SCRATCH_ARENA_GB", "24"))
                    * (1024**3)
                )
                self._scratch_arena_bytes = max(
                    required_bytes, configured_bytes
                )
                self._scratch_arena = torch.empty(
                    self._scratch_arena_bytes,
                    dtype=torch.uint8,
                    device=f"cuda:{self._device_id}",
                )
                scratch_tensors = self._scratch_views(
                    all_source_tensors, alignment=alignment
                )
            ...
            with refit_span(
                "setup_registration",
                status="cache_hit",
                metadata={"scratch_buffer_cache": "arena_rebind"},
            ):
                self._scratch_tensors = self._scratch_views(
                    all_source_tensors, alignment=256
                )
                self._scratch_sig = sig
                self._nixl.rebind_tensors(self._scratch_tensors)
            logger.info(
                "Rebound %d source-layout views into persistent %.2f GB "
                "scratch arena",
                len(self._scratch_tensors),
                self._scratch_arena_bytes / 1e9,
            )
        else:
            with refit_span(
                "setup_registration",
    ...
    def _scratch_views(
        ...
        *,
        ...
        """Create typed source-name views in one registered CUDA arena."""
        if self._scratch_arena is None:
            raise RuntimeError("scratch arena is not allocated")
        views: dict[str, torch.Tensor] = {}
        offset = 0
        for descriptor in source_tensors:
            offset = (offset + alignment - 1) // alignment * alignment
            end = offset + int(descriptor.size)
            if end > self._scratch_arena_bytes:
                raise RuntimeError(
                    "Source layout exceeds persistent scratch arena: "
                    f"need {end / 1e9:.2f} GB, capacity "
                    f"{self._scratch_arena_bytes / 1e9:.2f} GB. Increase "
                    "MX_SCRATCH_ARENA_GB before receiver initialization."
                )
            dtype = _DTYPE_MAP.get(descriptor.dtype, torch.bfloat16)
            element_size = torch.empty((), dtype=dtype).element_size()
            if int(descriptor.size) % element_size != 0:
                raise RuntimeError(
                    f"Tensor {descriptor.name!r} byte size {descriptor.size} "
                    f"is not divisible by dtype size {element_size}"
                )
            views[descriptor.name] = (
                self._scratch_arena.narrow(
                    0, offset, int(descriptor.size)
                )
                .view(dtype)
            )
            offset = end
    ...
    def receive_segment(
        ...
        *,
        ...
        # Must track the agent's actual backend: hardcoding UCX prepares the
        # transfer on a backend the agent does not have on AWS EFA (LIBFABRIC).
        backends = self._nixl.backends
