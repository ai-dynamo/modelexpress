...
See ``docs/RL/VERL_MX_OVERVIEW.md`` §3 ("Rank-to-rank publishing without
allgather") for the design rationale and how it maps to the Gen 2 verl
integration.
"""
...
class SliceOwnership:
    ...
    Args:
        ...
        is_expert: True if this is an MoE expert tensor (the planner
            knows to interpret ``shard_axis`` as the expert axis and
            ``local_shard_range`` as the owned-expert range).
        expert_axis: which axis enumerates experts (when ``is_expert``).
        owned_expert_ids: explicit list of expert IDs this rank owns.
            Used by receivers that need only a subset of experts (EP
            sharding on the inference side).
