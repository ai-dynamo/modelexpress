...
def _log_gpu_memory_stage(
    label: str, prev_used: int = 0, log: logging.Logger | None = None
) -> int:
    ...
    if log is not None:
        log.info(
            ...
            used / _GIB, total / _GIB, delta / _GIB,
        )
    return used
...
class MxModelLoader(BaseModelLoader):
    ...
    def load_model(
        ...
        prev = _log_gpu_memory_stage("Before model init (mx)", log=logger)
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            prev = _log_gpu_memory_stage("After initialize_model (mx)", prev, logger)
    ...
    def _load_as_target(
        ...
        prev = _log_gpu_memory_stage("After dummy load_weights (mx-target)", log=logger)

        # Process dummy weights to establish final tensor layout
        process_weights_after_loading(model, model_config, target_device)
        prev = _log_gpu_memory_stage("After process_weights_after_loading (mx-target)", prev, logger)
        ...
        prev = _log_gpu_memory_stage("After RDMA transfer (mx-target)", prev, logger)
        ...
        _log_gpu_memory_stage("After NIXL registration (mx-target)", prev, logger)
    ...
    def _load_as_source(
        ...
        prev = _log_gpu_memory_stage("After load_weights (mx-source)", log=logger)

        # Process weights FIRST, then register final tensors
        process_weights_after_loading(model, model_config, target_device)
        prev = _log_gpu_memory_stage("After process_weights_after_loading (mx-source)", prev, logger)

        # Register with NIXL + publish
        self._register_and_publish(model, target_device, device_id, model_name)
        _log_gpu_memory_stage("After NIXL registration (mx-source)", prev, logger)
