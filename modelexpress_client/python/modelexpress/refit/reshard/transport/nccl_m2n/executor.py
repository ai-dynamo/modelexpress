...
from .mesh import build_tp_meshes, shard_dim_from_layout, tile_shape
from .runtime import (
    _M2nCall,
    _M2nLane,
    _M2nLaneBatch,
...
def _torch_dtype_from_table(dtype_name: str) -> Any:
    """Parse a ``LayoutTensor.dtype`` string such as ``torch.bfloat16``."""
    import torch

    name = dtype_name.replace("torch.", "", 1)
    dtype = getattr(torch, name, None)
    if dtype is None or not isinstance(dtype, torch.dtype):
        raise ValueError(f"unsupported layout-table dtype {dtype_name!r}")
    return dtype
...
def build_reshard_params(
    ...
    Shape and shard layout come from the shared ``LayoutTable``. Local
    ...
    for tensor in table.tensors:
        ...
        if param is None:
            raise RuntimeError(
                f"layout table parameter {tensor.name!r} is not present in the local model; "
        ...
        if param.dtype != expected_dtype:
            raise ValueError(
                ...
                f"layout table dtype {expected_dtype}"
            )
        params.append(
            ReshardParam(
                name=tensor.name,
                global_shape=tuple(tensor.shape),
                shard_dim=shard_dim_from_layout(tensor),
                local_tensor=param,
            )
        )
    return params
