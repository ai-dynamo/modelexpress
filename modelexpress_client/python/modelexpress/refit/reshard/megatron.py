...
class MegatronTargetSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.global_shape or any(int(extent) <= 0 for extent in self.global_shape):
            raise ValueError(
...
class MegatronTargetLayout:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.tp_rank < self.tp_size:
            raise ValueError(
                f"tp_rank {self.tp_rank} is outside [0, {self.tp_size})"
            )
...
def _role_axis(spec: MegatronTargetSpec) -> int | None:
    ...
    if spec.shard_axis is not None:
        ...
        axis = (
            1
            if spec.descriptor_extras.get("expert_layout") == "leading_axis"
            else 0
        )
