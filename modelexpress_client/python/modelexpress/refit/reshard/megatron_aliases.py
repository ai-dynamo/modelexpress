    ...
    return PublishedTensor(
        ...
        shards=[
            PublishedShard(
                ...
                shape=local_shape,
                # Same contract as megatron_publisher: without a digest here the
                # receiver's verify gate reports skipped_no_digest and proves
                # nothing. Aliased tensors are the majority on a fused-QKV,
                # gated-MLP MoE, so omitting them left the gate blind.
...
def _build_gated_aliases(
    ...
    shard_range = (
        (source_rank * half, (source_rank + 1) * half)
        if source_size > 1
        else None
    )
    return [
        ...
        for tensor, shards, start in (
            ...
            shards.append(
                PublishedShard(
                    ...
                    # q/k/v are narrow() views of the fused tensor, so the digest
                    # must be taken over the view a receiver actually reads, not
                    # over the parent.
                    digest=tensor_digest(tensor) if VERIFY else None,
                )
            )
...
def build_hf_aliases(
    ...
    for item in items:
        if item.role == "qkv_column":
            ...
            continue
        if item.role in {"gated_mlp_column", "expert_column"} and len(
            item.hf_names
        ) == 2:
