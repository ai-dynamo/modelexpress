...
class MxTrainingPublisher:
    ...
    def __init__(
        ...
        # Per-step tracking for publish_layer(): each call produces a new
        # mx_source_id since the layer's tensor set differs. mark_ready()
        # must flip ALL of them to READY, not just the last one, otherwise
        # READY-only discovery sees a partial weight set. Cleared when the
        # step changes.
        self._pending_source_ids: list[str] = []
    ...
    def publish_layer(
        ...
        # Track this layer's source_id under the current step so mark_ready()
        # can flip all of them to READY together. Reset the list on step
        # boundary — only the current step's layers should transition.
        if self._pending_step != step:
            self._pending_source_ids = []
            self._pending_step = step
        self._pending_source_ids.append(self._mx_source_id)
        logger.debug(
    ...
    def mark_ready(self, worker_rank: int = 0) -> bool:
        ...
        For ``publish_weights()`` (whole-step single publish), flips the one
        ``mx_source_id``. For ``publish_layer()`` (per-layer streamed
        publish), flips every ``mx_source_id`` recorded for the current
        step — otherwise earlier layers stay in ``SOURCE_STATUS_INITIALIZING``
        and READY-only discovery sees only the last layer's tensors.
        """
        if self._mx_source_id is None:
            raise RuntimeError("No weights published yet; call publish_weights() first")

        # Per-step list is populated only by publish_layer(); publish_weights()
        # leaves it empty so we fall back to the single mx_source_id.
        source_ids = self._pending_source_ids or [self._mx_source_id]
        ok = True
        for sid in source_ids:
            ok = self._client.update_status(
                mx_source_id=sid,
                worker_id=self._worker_id,
                worker_rank=worker_rank,
    ...
    def shutdown(self) -> None:
        ...
        if self._client is not None:
            ...
            self._client = None
        self._initialized = False
        # Reset registration state too — without this, a publisher that gets
        # reinitialized would skip the NIXL register_tensors call on the next
        # publish_weights() and publish stale/empty descriptors against a
        # fresh NixlTransferManager.
        self._registered = False
