...
**The `source_load` signal (stateless).** `source_load` is produced by the source about *itself* and only passed through by the server, so MX servers stay stateless behind a load balancer (no server-side counters; every replica ranks identically). The default provider samples the source's RDMA NIC utilization from its own InfiniBand port counters (`/sys/class/infiniband/<dev>/ports/1/counters`) over the heartbeat interval and reports it on each `UpdateStatus`; the server stamps it onto `SourceInstanceRef`. `make_source_load_provider` (`nic_metrics.py`) is the seam where an inference-runtime provider (vLLM/SGLang serving load, or a max-blend) could replace the NIC provider without any server/proto/selector change — the wire contract is just the normalized `source_load` value.
