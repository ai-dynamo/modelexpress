...
│       │   ├── load_tracker.rs         # TTL-decayed per-source load estimate (active_transfers)
...
│       ├── source_selection.py         # P2P source-ordering policies (random, rendezvous_hash, load_aware)
...
|---|---|---|---|
| p0 | `RdmaStrategy` | NIXL available | `ListSources(READY)`, filter by `worker_rank` and by runtime `accelerator` (dropping sources whose non-empty accelerator differs from the target), order the survivors via the configured `SourceSelector` (`MX_P2P_SOURCE_SELECTOR`: `random` default, `rendezvous_hash`, or `load_aware`), then fetch candidate metadata, trying candidates (max 3). Filtering on `SourceInstanceRef.accelerator` before the retry slice keeps incompatible sources from exhausting the retry budget ahead of a compatible one; a post-`GetMetadata` accelerator re-check remains as defense-in-depth. Candidate retry covers pre-transfer metadata misses (no metadata found, fetch error) only. Once the RDMA receive starts, a `SourceTransferError` or `ManifestMismatchError` aborts RDMA and the chain falls through to the next strategy (GDS, then disk), not the next source. |
...
After `RdmaStrategy` filters listed READY sources to the target's `worker_rank` and to a compatible runtime `accelerator`, it ranks the surviving candidates through a `SourceSelector` (`source_selection.py`) before slicing to `MAX_SOURCE_RETRIES`. Selectors are scoring-based: `ScoredSelector` subclasses implement `score(candidate, context)` and the base orders by descending score. Three policies ship today, resolved by name through a small registry (`MX_P2P_SOURCE_SELECTOR`, default `random`, unknown values fall back to `random`):
...
- `load_aware` — biases the `rendezvous_hash` ordering away from busy sources: `score = unit_hash(target, candidate) − w_load · normalized_load`, where the load term is each candidate's server-estimated `active_transfers` normalized by the maximum across the candidate set, and `w_load` comes from `MX_P2P_LOAD_WEIGHT` (default `1.0`). When load is uniform or unavailable (older servers, tracking disabled) every penalty is equal and ordering collapses exactly to `rendezvous_hash`, so it is never worse than the deterministic baseline. It targets the single-source convergence during live fan-out (or RL-style repeated weight sync) that a load-blind hash cannot avoid when a source is persistently available.

The `load_aware` load signal is produced server-side with no new per-transfer RPCs and no `WorkerMetadata`/backend-schema change. The central coordinator already observes every selection as a `GetMetadata(mx_source_id, worker_id)` call (a target fetches the chosen source's metadata immediately before pulling), so `SourceLoadTracker` (`modelexpress_server/src/p2p/load_tracker.rs`) counts those calls per source within a sliding TTL window (`MX_P2P_SOURCE_LOAD_TTL_SECS`, default 60s) and stamps the count onto each `SourceInstanceRef` as the new `active_transfers` field in `ListSources`. It is transient in-memory state, scoped to a single central-coordinator server. With replicated servers (e.g. the Helm production profile's `replicaCount: 3`) the tracker is per-replica and gRPC channel pinning gives each client a ~1/N view of peer selections — steering weakens proportionally (toward `rendezvous_hash` behavior, never wrong routing); a Redis-shared TTL window restoring the full signal across replicas is the scoped follow-up. the tensor RDMA path has no source-side completion signal, so the TTL window — rather than an explicit release — bounds a selection's contribution to the count. The counts are deliberately not persisted to the metadata backend: on a server restart they reset to zero, so `load_aware` collapses to `rendezvous_hash` (its tested zero-load path) and the window repopulates from live selections within one TTL — a transient quality dip, never incorrectness. Persistence would buy little (the signal decays within the TTL, comparable to a crash-restart itself) and cost a backend write per `GetMetadata` on the hot path.

The selector controls ordering only; the retry budget and the rule that a transfer failure does not fall back to another source are unchanged. Selectors read the few fields they need (`worker_rank`, `worker_id`, `identity.model_name`, and `active_transfers` for `load_aware`) directly off the live `LoadContext` and candidate refs, so there is no parallel context type. Topology-aware policies remain deferred (they still need node/rack topology signals); the same scoring interface accepts them as drop-ins. Selection decisions are emitted as structured logs, and an opt-in Prometheus collector (`metrics.py`, `MX_METRICS_ENABLED=1`) re-emits them as `mx_p2p_*` metrics (including `mx_p2p_active_transfers`) for benchmarking different schemes.
...
On-cluster (8×B200 nodes, InfiniBand RDMA; vLLM `--load-format modelexpress`, TP=1, central-coordinator backend), both policies were run against pre-warmed source pools that grow as targets become sources. Across Qwen2.5-0.5B (0.99 GB) and Qwen2.5-7B (~15 GB), 42 real cross-node NIXL RDMA transfers completed and **bandwidth was policy-independent** (~180 Gbps for the 0.5B transfers, ~211 Gbps for the larger 7B transfers as setup amortizes). So `rendezvous_hash`'s win is determinism and low disruption at no balance or bandwidth cost; the `load_aware` policy (below) closes the live-fan-out gap where deterministic hashing can pile onto an always-present source.

#### Load-aware efficacy (measured)

`load_aware` reacts to *live* per-source load, so its benefit cannot be shown by the identity-only offline sim above — a timed model is required. `modelexpress_client/python/benchmarks/source_selection_sim.py` drives the real selector code over a processor-sharing fan-out: `M` pre-warmed sources (one persistent), `N` targets arriving over time, each transfer occupying a source and splitting its bandwidth, and each target ranking on the current per-source `active_transfers`. Across the 4×20, 4×40, and 8×32 configs, `load_aware` gives the **lowest max-source share, the most even distribution (coefficient of variation), and the shortest fan-out makespan and p99** of the three policies — e.g. at 8×32 it cuts max-source share from `rendezvous_hash`'s ~0.22 to ~0.16 and makespan from ~70 to ~53 (simulated seconds) — while remaining identical to `rendezvous_hash` when load is uniform. `tests/test_source_selection_sim.py` guards this direction (and the no-load parity) in CI.

On-cluster (B200 nodes, InfiniBand RDMA; vLLM `--load-format modelexpress`, TP=1, central-coordinator `kubernetes` backend, P2P-endpoint path; the patched server built from this branch), two experiments against `M=4` pre-warmed sources:

*Fan-out distribution (Qwen2.5-0.5B, 12 targets launched together, pool grows as targets become sources):* `random` converged 6 of 12 targets onto one source (max-share 0.50, 5 distinct sources used), `rendezvous_hash` spread 3/12 (0.25, 7 distinct), and `load_aware` spread near-ideally — max 2/12 (0.167) across 11 distinct sources. Makespan and per-transfer times were policy-flat at this size: a 0.99 GB transfer takes ~50 ms at ~180 Gbps, so no contention exists to remove — the win is eliminated convergence at zero cost.

*Contention / perf gain (Qwen2.5-7B, 15.2 GB per transfer, 12 targets released by a synchronized start barrier against a fixed 4-source pool, 2 trials per policy):* concurrent pulls sharing one source's IB rail visibly split bandwidth (~215 Gbps uncontended → 57–99 Gbps under a 6-deep pile). `rendezvous_hash` piled 6/4/2/0 and 4/4/3/1; `load_aware` capped the pile at 4 (3/3/2/2 and 4/4/2/2). Where selections had even ~1–3 s of arrival spread, steering cut median transfer time 1.43 s → 0.57 s (2.5×), tail 2.13 s → 0.97 s (2.2×), and raised mean per-transfer bandwidth ~122 → ~199 Gbps (+63%); with fully simultaneous picks (several in the same second, before any peer's selection could register at the server) it degraded gracefully to rendezvous-equivalent — the designed stateless fallback, never worse. At Qwen3-32B (~65 GB per transfer, every transfer overlapping, all three policies), `rendezvous_hash`'s 5-deep pile served at 48–52 Gbps and stretched its targets past 10 s, and `random` (4/4/3/1) left only one target uncontended (median 8.0 s, mean 80 Gbps); `load_aware` (4/3/3/2) beat both on every metric — worst transfer 7.1 s vs 10.9 s rendezvous (1.54×), median 6.0 s vs 8.0 s random (1.34×, +35% mean bandwidth) — under saturation, balance compresses the straggler that gates fleet readiness. (`random` and `rendezvous_hash` are the same balls-into-bins process for a single wave of distinct targets; which one draws the deeper pile varies per run — `random` at 7B across two trials swung from median 1.08 s to 0.56 s on draw luck alone, while `load_aware`'s trials stayed tight at 0.57 s/0.71 s and beat both baselines pooled: 0.58 s vs 0.82 s (`random`) vs 1.08 s (`rendezvous_hash`) median.) An AIPerf serving-impact round (first 8 minutes of the Mooncake arXiv production trace, fixed schedule, ~5.7 req/s against the 4-source fleet, 12-target wave launched mid-replay) additionally showed the transfer gains hold under live inference load (`load_aware` max pile 2 across 8 sources, worst transfer 0.84 s vs `rendezvous_hash`'s 1.62 s) while serving TTFT/ITL/goodput stayed policy-flat in aggregate and per-timeslice — at this transfer size, scale-up pulls are SLA-invisible to live inference. Across all rounds: 120/120 RDMA transfers succeeded, zero fallbacks, uncontended bandwidth policy-independent. Summary: transfer-tail reduction of 1.5–2.5× under concurrent fan-out, bounded below by `rendezvous_hash`. Larger transfers make the gain unconditional (small models finish before transfers overlap, so there is nothing to remove; at 32B every pick was contended) and scale the absolute seconds saved linearly with bytes per rank, while the multiplier peaks in the partial-overlap regime and settles under saturation toward a bound set by the fan-out ratio N/M.
...
4. **Target discovers**: Calls `ListSources(identity, status=READY)`, which returns only READY workers whose `updated_at` is still within `MX_HEARTBEAT_TIMEOUT_SECS`, then filters by `worker_rank` and by compatible runtime `accelerator`
5. **Target fetches on demand**: Calls `GetMetadata(mx_source_id, worker_id)` for the chosen candidate. Accelerator-incompatible sources were already dropped during `ListSources` handling using `SourceInstanceRef.accelerator`; this step re-checks the authoritative `WorkerMetadata.accelerator` as defense-in-depth and skips the source before target preparation if both source and target publish non-empty, different values. Empty accelerator metadata is treated as unknown and accepted for backward compatibility. Auto-detects P2P mode if `worker_grpc_endpoint` is populated - fetches tensors or artifact manifest metadata from the source worker's `WorkerService` and NIXL metadata via the listen thread instead of from the central server.
6. **Target transfers**: Executes RDMA reads from source; for cache artifacts, it prepares one source chunk lease at a time, receives into target registered DRAM, verifies CRC32C, writes to target-local staging, releases the lease, then installs the staged tar into the runtime cache directory. Once target preparation starts, a `SourceTransferError` aborts RDMA and the chain falls through to the next strategy (GDS, then disk), not the next source; candidate retry (max 3) covers only pre-transfer metadata misses
