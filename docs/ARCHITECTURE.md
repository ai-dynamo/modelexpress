...
|---|---|---|---|
| p0 | `RdmaStrategy` | NIXL available | `ListSources(READY)`, filter by `worker_rank`, order via the configured `SourceSelector` (`MX_P2P_SOURCE_SELECTOR`: `random` default or `rendezvous_hash`), try up to `MX_P2P_MAX_SOURCE_RETRIES` candidates (default 3). Candidate retry covers metadata misses and transfer failures. If a transfer failure happens after target tensors may have been mutated, the target model is reinitialized through the engine adapter before the next source is tried. If the last source fails after mutation, the outer strategy chain reinitializes the target again before falling through to the next strategy (`ModelStreamerStrategy`, then GDS, then disk). |
...
After `RdmaStrategy` filters listed READY sources to the target's `worker_rank`, it ranks the candidates through a `SourceSelector` (`source_selection.py`) before slicing to `MX_P2P_MAX_SOURCE_RETRIES` (default 3). Selectors are scoring-based: `ScoredSelector` subclasses implement `score(candidate, context)` and the base orders by descending score. Two policies ship today, resolved by name through a small registry (`MX_P2P_SOURCE_SELECTOR`, default `random`, unknown values fall back to `random`):
...
During transfer, `ManifestMismatchError` is raised if source and target tensor names or sizes don't match (a likely symptom of a rolling update where pods run different image versions). The receive path converts it, like any receive failure, into a `SourceTransferError`. `RdmaStrategy` treats the failed source as a candidate miss, rolls back strategy-owned NIXL state, reinitializes the target model if tensors may have been mutated, and tries the next ranked source until `MX_P2P_MAX_SOURCE_RETRIES` is exhausted.
...
6. **Target transfers**: Executes RDMA reads from source; for cache artifacts, it prepares one source chunk lease at a time, receives into target registered DRAM, verifies CRC32C, writes to target-local staging, releases the lease, then installs the staged tar into the runtime cache directory. On `SourceTransferError` tries the next candidate up to `MX_P2P_MAX_SOURCE_RETRIES` times
...
| `MX_P2P_MAX_SOURCE_RETRIES` | `3` | Maximum READY source candidates `RdmaStrategy` tries for one target load before falling through to the next load strategy. Non-positive values fall back to `3` |
