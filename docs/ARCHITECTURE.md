...
The on-cluster efficacy run is a fast-follow on the same `dynamo-nscale-dev-cluster` (8×B200, InfiniBand RDMA): a small model at TP=1 packed onto fragmented free GPUs, pre-warming `M=4` sources including one persistent source, then launching `N` targets in the tens under each policy (distinct `MX_METRICS_SCHEME`). It compares `mx_p2p_source_selections_total` by `source_worker_id` (utilization balance / convergence), `mx_p2p_active_transfers` over time, fan-out makespan, and `mx_p2p_transfer_seconds` tails. Pass condition: `load_aware` lowers and stabilizes the max-source share and improves makespan and tail transfer time under high fan-out versus `rendezvous_hash`, with no regression in average transfer time or fallback rate, and parity under uniform load.
