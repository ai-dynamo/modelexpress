...
| `MX_TVM_FFI_CACHE_ARTIFACT_MODE` | `standalone` | SGLang TVM-FFI cache source mode. `standalone` uses the dedicated source type; `triton` packages it into the Triton artifact so a new client can be validated against an older server. Source and target clients must use the same mode |
