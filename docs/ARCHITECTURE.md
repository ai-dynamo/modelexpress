...
- `topology_aware` — locality-first spreading for the RDMA fabric. Ranks each candidate by the **narrowest topology domain the target and source share** (same rail → rack → block → cross-rack), with the `rendezvous_hash` digest as an intra-tier tiebreak so equidistant peers still spread. The level hierarchy is not hard-coded: it comes from `MX_P2P_TOPOLOGY_LEVELS` (the cluster's Dynamo `ClusterTopology` `spec.levels`), and per-source domain values from each candidate's published `topology` metadata — so MX consumes existing Dynamo/Grove node labels. NVLink is not modeled (co-located replicas let NIXL auto-select the NVLink backend). See below.
...
**Topology mechanism.** A worker reports its RDMA-fabric location as a `{level: domain_value}` map (e.g. `{"rack": "r3", "rail": "leaf2", "host": "node7"}`), read from the environment (`MX_P2P_TOPOLOGY`, populated by the operator from node labels) by `topology.py` and published **once at registration** on `WorkerMetadata.topology`. Because it is static per node, it rides the existing publish path (not the heartbeat) — the server passes it straight through onto `SourceInstanceRef.topology`, keeping it stateless. `TopologyAwareSelector.score` returns `(shared_depth, tiebreak)`: `shared_depth` is the deepest configured level at which the target and candidate maps agree, so `order()` puts the closest source first. Setting `MX_P2P_TOPOLOGY_LOAD_WEIGHT > 0` swaps the intra-tier tiebreak for `unit_hash − w·source_load` (read defensively), so **within a locality tier** selection also steers away from busy sources — this is how topology- and load-aware selection compose without either depending on the other. Missing levels, missing node topology, or an old server that predates the field all collapse the policy to `rendezvous_hash`, so it is never worse than the deterministic baseline. Meaningful benefit needs a topology-diverse allocation (sources across multiple racks/rails); packed into one rack all sources are equidistant and the policy collapses to `rendezvous_hash`.
