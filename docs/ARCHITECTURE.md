...
|---|---|---|---|
| p0 | `RdmaStrategy` | NIXL available | `ListSources(READY)`, filter by `worker_rank`, runtime `accelerator`, and the optional hard topology boundary `MX_P2P_TOPOLOGY_FILTER_LEVEL`, order the survivors via the configured `SourceSelector` (`MX_P2P_SOURCE_SELECTOR`: `random` default, `rendezvous_hash`, `load_aware`, or `topology_aware`), then try candidates (max 3). Filtering before the retry slice prevents incompatible or unreachable sources from exhausting the retry budget; a post-`GetMetadata` accelerator check remains as defense-in-depth. Before preparing target tensors, P2P sources must serve a manifest for the selected runtime `worker_id`; generation mismatches and transfer failures retry the next candidate, reinitializing the target first when it may have been mutated. |
...
Mutated strategy failures close any exposed weight iterator before recovery, clearing ModelStreamer, InstantTensor, and GDS staging resources while the failure is still local to the strategy. The chain then clears exception tracebacks and asks the engine adapter to restore a fresh model state. SGLang and vLLM preserve the externally retained model root as an empty shell: they release its parameter graph, collect Python references, empty the accelerator cache, initialize one fresh model, and move that state back into the retained root. This prevents retry or native fallback from holding two complete models at once. VMM changes allocation and registration mechanics only; retry uses the same root-reuse lifecycle with `MX_VMM_ARENA` on or off.
