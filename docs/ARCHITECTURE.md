...
| `MX_ARTIFACT_COMPILE_CONFIG_DIGEST` | `""` (unset) | Feeds the torch compile cache `SourceIdentity`, partitioning artifact discovery by compile configuration. Unset leaves the field empty, removing only compile-configuration partitioning, so workers whose other identity fields match share one pool. See [Pairing workers by compile configuration](DEPLOYMENT.md#pairing-workers-by-compile-configuration) |
