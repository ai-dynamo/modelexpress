...
| `MX_P2P_METRICS_ENABLED` | `0` | Opt-in Prometheus metrics for source-selection benchmarking. `1` enables the `mx_p2p_*` collectors (requires the `metrics` extra, `prometheus-client`). Off by default; selection signals are always emitted as structured logs regardless. |
| `MX_P2P_METRICS_PORT` | (unset) | With metrics enabled, serve a pull `/metrics` endpoint on this port. |
| `MX_P2P_METRICS_PUSHGATEWAY` | (unset) | With metrics enabled, push to this Pushgateway host:port (for short-lived load-only processes). |
| `MX_P2P_METRICS_SCHEME` | `""` | Optional run/scheme label added to every `mx_p2p_*` metric, so multiple schemes compare on one dashboard. |
