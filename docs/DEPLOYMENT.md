...
| `MX_METRICS_ENABLED` | `0` | Opt-in Prometheus metrics collector for the client. `1` enables the collectors (requires the `metrics` extra, `prometheus-client`, which every client image in this repo installs). Off by default; selection signals are always emitted as structured logs regardless. See [METRICS.md](METRICS.md). |
