...
| `UCX_MEM_EVENTS` | `n` (MX default; UCX default is `y`) | ModelExpress sets this to `n` before NIXL agent creation unless already set. Disables UCX's memory-event tracking (mem hooks / VMM registration-cache invalidation), roughly halving NIXL agent creation time. Safe because ModelExpress registers all transferred tensors explicitly (`MX_POOL_REG` / `MX_VMM_ARENA` / per-tensor reg) instead of relying on UCX to catch unregistered allocations behind its back. Set explicitly to override. |
