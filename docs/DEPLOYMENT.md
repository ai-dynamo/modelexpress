...
| `MX_P2P_TOPOLOGY_LEVELS` | (unset) | For `topology_aware`. Comma-separated topology levels, broad → narrow, matching the cluster's Dynamo `ClusterTopology` `spec.levels`, e.g. `region,zone,datacenter,block,rack,rail,host`. The exact RDMA ordering (e.g. whether rail sits above rack) is fabric-dependent, so it is configured rather than fixed in code. Unset → `topology_aware` collapses to `rendezvous_hash`. |
| `MX_P2P_TOPOLOGY` | (unset) | For `topology_aware`. This node's `{level: value}` domain map as JSON, e.g. `{"rack":"r3","rail":"leaf2","host":"node7"}`, populated by the operator from the node's topology labels. Published once at registration and surfaced on `SourceInstanceRef.topology`. Unset/unparseable → this node shares no domain (rendezvous ordering). |
| `MX_P2P_TOPOLOGY_LOAD_WEIGHT` | `0.0` | For `topology_aware`. When `> 0`, the within-tier tiebreak becomes `unit_hash − w·source_load` (source_load read defensively), so within a locality tier selection also steers away from busy sources — composing topology- and load-aware selection. `0` keeps the pure rendezvous jitter. Clamped to `≥ 0`. |
