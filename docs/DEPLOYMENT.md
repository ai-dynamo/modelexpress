...
| `MX_P2P_TOPOLOGY_FILTER_LEVEL` | (unset) | Optional hard RDMA reachability boundary, independent of the ordering policy. Only sources whose published topology value matches this node at the named level are eligible. Use an operator-defined `fabric` domain or an existing level such as `block`. Missing local/source metadata rejects the source and falls through to local loading. Unset preserves backward-compatible soft topology preference. |
