...
| `MX_P2P_SOURCE_LOAD_TTL_SECS` | `60` | Server-side. Sliding-window TTL over which a source's recent `GetMetadata` selections are counted into the `active_transfers` load estimate returned by `ListSources` (the signal `load_aware` consumes). Shorter = more reactive but noisier; longer = smoother but slower to decay. |
