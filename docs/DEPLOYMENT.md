...
| `MX_P2P_SOURCE_SELECTOR` | `random` | P2P source-ordering policy for the RDMA load path. `random` (behavior-preserving default; local-RNG shuffle), `rendezvous_hash` (stateless deterministic spreading via HRW hashing; stable across restarts and minimally disrupted by source-set changes), or `load_aware` (`rendezvous_hash` biased away from busy sources by the server-estimated `active_transfers`; collapses to `rendezvous_hash` when load is uniform or unavailable). Unknown values log a warning and fall back to `random`. Ordering only — the `MAX_SOURCE_RETRIES=3` retry budget is unchanged. |
| `MX_P2P_LOAD_WEIGHT` | `1.0` | Weight of the load penalty in the `load_aware` selector (`score = unit_hash − w_load · normalized_load`). `0` disables the penalty, making `load_aware` behave exactly like `rendezvous_hash`. Ignored by other policies. |
| `MX_P2P_SOURCE_LOAD_TTL_SECS` | `60` | Server-side. Sliding-window TTL over which a source's recent `GetMetadata` selections are counted into the `active_transfers` load estimate returned by `ListSources` (the signal `load_aware` consumes). Shorter = more reactive but noisier; longer = smoother but slower to decay. The window is in-memory: a server restart resets it to zero, making `load_aware` behave as `rendezvous_hash` until it repopulates (within one TTL). |
| `MX_METRICS_ENABLED` | `0` | Opt-in Prometheus metrics collector for the client. `1` enables the collectors (requires the `metrics` extra, `prometheus-client`). The P2P source-selection group (`mx_p2p_*`, including `mx_p2p_active_transfers`) ships today; the collector is generic so other client metrics can be added later. Off by default; selection signals are always emitted as structured logs regardless. |
