...
| `MX_TVM_FFI_CACHE_ARTIFACT_MODE` | `standalone` | SGLang TVM-FFI cache publication mode. `standalone` uses the dedicated `TVM_FFI_CACHE` source type and requires a server with that enum. `triton` packages TVM-FFI as an additional root in the existing Triton artifact for client-only validation against an older server. Source and target clients must use the same mode. |
