...
When using the Helm chart, enable `serviceAccount.rbac.enabled=true` so the
server can manage `ModelMetadata` and `ModelCacheEntry` resources. For a
standalone Kubernetes deployment, apply `examples/crds.yaml` and
