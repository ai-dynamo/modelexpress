...
The Helm chart installs both the `ModelMetadata` P2P CRD and the
`ModelCacheEntry` registry CRD automatically. Enable
`serviceAccount.rbac.enabled=true` so the server can manage their resources. For
a standalone Kubernetes deployment, apply `examples/crds.yaml` and
