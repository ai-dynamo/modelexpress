...
path with `model_config.enforce_eager=True`; otherwise they use the canonical
S3 path. The peer transfer copies the registered runtime representation without
rerunning post-load processing. Eager installation then refreshes q/k/v host
scale mirrors and invalidates FlashInfer launch-scale caches for recomputation
on the next forward. This does not clear stored KV entries; retaining quantized
KV entries across a change to their K/V scales remains unsupported.
