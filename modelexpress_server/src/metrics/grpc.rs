...
pub enum RpcOutcome {
    ...
    NotFound,
    /// The metadata backend failed. This is the variant a status-code-derived
    /// metric cannot see, and the reason this module exists.
    BackendError,
...
mod tests {
    ...
    fn routed_paths(proto: &str) -> Vec<(String, String)> {
    ...
    fn every_routed_proto_rpc_has_a_method_label_arm() {
        ...
        let mut checked = 0;
        for proto in PROTOS {
            for (service, path) in routed_paths(proto) {
                ...
                assert_ne!(
                    ...
                    "{path} has no method_label arm, so it would be recorded as `other` \
                     -- and if it is a streaming RPC it would also be timed at the head"
                );
                checked += 1;
            }
        }
    ...
    /// The two `stream` RPCs in model.proto, plus the long-lived health watch,
    /// must be excluded. If a new streaming RPC is added to a proto without being
    /// listed here, it will silently report `ok` with a stream-setup duration.
    #[test]
