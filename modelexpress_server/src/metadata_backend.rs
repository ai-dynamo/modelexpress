...
//! - **Kubernetes**: CRDs for native K8s integration
//!
...
//! lightweight directory entries: endpoints, agent names, and status.
...
pub trait MetadataBackend: Send + Sync {
    ...
    /// Get metadata for one specific worker.
    ...
    /// `status_filter`: if `Some(s)`, return only workers where any worker has status `s`.
    ...
    /// Patch the status of a specific worker.
