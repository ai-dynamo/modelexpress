...
fn split_run(
    ...
    while remaining > 0 {
        let row = if elems_per_row > 0 { pos / elems_per_row } else { 0 };
        let col = if elems_per_row > 0 { pos % elems_per_row } else { 0 };

        let shard = match shards.iter().find(|s| s.row_start <= row && row < s.row_end) {
            Some(s) => s.clone(),
