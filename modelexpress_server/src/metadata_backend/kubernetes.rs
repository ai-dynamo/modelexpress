...
use crate::k8s_types::{
    sanitize_model_name, ModelMetadata, ModelMetadataPhase, ModelMetadataSpec,
...
impl KubernetesBackend {
    ...
    async fn read_tensor_configmap(
        &self,
        cm_name: &str,
    ) -> MetadataResult<Vec<TensorRecord>> {
        let api = self.configmap_api();
...
impl MetadataBackend for KubernetesBackend {
    ...
    async fn publish_metadata(
        ...
        let worker_records: Vec<WorkerRecord> = workers.into_iter().map(WorkerRecord::from).collect();
        ...
        let mut all_workers: Vec<WorkerStatus> = current
            .status
            .map(|s| s.workers)
            .unwrap_or_default();
        ...
        for worker_status in status.workers {
            ...
            let tensors = if let Some(cm_name) = &worker_status.tensor_config_map {
                match self.read_tensor_configmap(cm_name).await {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to read tensor ConfigMap '{}': {}",
                            cm_name, e
                        );
                        Vec::new()
                    }
    ...
    async fn list_models(&self) -> MetadataResult<Vec<String>> {
        ...
        let models: Vec<String> = crs
            .items
            .into_iter()
            .map(|cr| cr.spec.model_name)
            .collect();
