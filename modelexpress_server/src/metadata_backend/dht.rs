...
struct WorkerRecordJson {
    ...
    #[serde(default)]
    worker_grpc_endpoint: Option<String>,
    #[serde(default)]
...
impl From<WorkerRecord> for WorkerRecordJson {
    fn from(r: WorkerRecord) -> Self {
        ...
        let transfer_engine_sid = match &r.backend_metadata {
            BackendMetadataRecord::TransferEngine(sid) => Some(sid.clone()),
            _ => None,
        };
        Self {
            ...
            worker_grpc_endpoint: if r.worker_grpc_endpoint.is_empty() {
                None
            } else {
                Some(r.worker_grpc_endpoint)
            },
...
impl From<WorkerRecordJson> for WorkerRecord {
    fn from(j: WorkerRecordJson) -> Self {
        Self {
            ...
            worker_grpc_endpoint: j.worker_grpc_endpoint.unwrap_or_default(),
        }
    }
}
...
impl MetadataBackend for DhtBackend {
    ...
    async fn publish_metadata(
        ...
        // Write the worker's record
        let rank = worker.worker_rank;
        let record: WorkerRecordJson = WorkerRecord::from(worker).into();
    ...
    async fn remove_worker(&self, source_id: &str, worker_id: &str) -> MetadataResult<()> {
        // Read the rank directory for this worker_id, remove each rank record
        if let Some(dir) = self
            .get_json::<WorkerDirectoryJson>(worker_directory_key(source_id, worker_id))
            .await?
        {
            for rank in &dir.ranks {
                self.dht_remove(worker_key(source_id, worker_id, *rank))
                    .await?;
            }
        }
        self.dht_remove(worker_directory_key(source_id, worker_id))
            .await?;

        // Remove from per-source instances directory
        if let Some(existing) = self
            .get_json::<InstancesDirectoryJson>(instances_key(source_id))
            .await?
        {
            let worker_ids: Vec<String> = existing
                .worker_ids
                .into_iter()
        ...
        debug!("Removed worker '{worker_id}' from source '{source_id}'");
        Ok(())
    }
...
mod tests {
    ...
    fn make_test_worker(rank: u32, endpoint: &str, agent: &str) -> WorkerMetadata {
        WorkerMetadata {
            worker_rank: rank,
    ...
    fn test_worker_record_json_roundtrip() {
        let record = WorkerRecord {
            worker_rank: 0,
            backend_metadata: BackendMetadataRecord::Nixl(Vec::new()),
            ...
            worker_grpc_endpoint: "10.0.0.1:5556".to_string(),
        };
    ...
    async fn test_publish_get_remove() {
        ...
        let worker = make_test_worker(0, "10.0.0.1:5555", "test-agent");

        node.publish_metadata(&identity, worker_id, worker)
            .await
            .expect("publish");
    ...
    async fn test_incremental_publish_same_worker_id() {
        ...
        // Publish rank 0
        let mut w0 = make_test_worker(0, "10.0.0.1:5555", "agent-0");
        w0.status = 1;
        node.publish_metadata(&identity, worker_id, w0)
            .await
            .expect("pub w0");

        // Publish rank 1 (incremental - same worker_id)
        let mut w1 = make_test_worker(1, "10.0.0.2:5556", "agent-1");
        w1.status = 1;
        node.publish_metadata(&identity, worker_id, w1)
            .await
            .expect("pub w1");
    ...
    async fn test_multiple_worker_ids_same_source() {
        ...
        node.publish_metadata(
            &identity,
            wid_a,
            make_test_worker(0, "10.0.0.1:5555", "agent-0"),
        )
        .await
        .expect("pub wid_a");
        node.publish_metadata(
            &identity,
            wid_b,
            make_test_worker(1, "10.0.0.2:5555", "agent-1"),
        )
        .await
        .expect("pub wid_b");
