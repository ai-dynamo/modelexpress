...
2. **ModelExpress CRDs** installed (one-time, requires cluster-admin):
   ...
   ```
   Installs `ModelMetadata` (P2P worker coordination) and `ModelCacheEntry` (model-download registry) in one pass.
3. **PVC** with the model weights pre-downloaded (the YAMLs expect a PVC named `shared-model-cache`; change `spec.pvcs` and `volumeMounts` to match your setup).
